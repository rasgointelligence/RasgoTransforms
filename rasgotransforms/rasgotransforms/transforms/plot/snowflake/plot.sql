...
with
    ...
    joined as (
        select
            ...
            {%- for aggregation_type in aggs %}
            {{ aggregation_type|lower|replace('_', '')|replace('distinct', '') }} (
                {{ 'distinct ' if 'distinct' in aggregation_type|lower else '' }} source_query.{{ column }}
            ) as {{ cleanse_name(aggregation_type + '_' + column) }},
            {%- endfor %}
            {%- endfor %}
    ...
    tidy_data as (
        select
            ...
            dateadd(
                'second', -1, dateadd('month', 3, {{ x_axis }}_min)
            ) as {{ x_axis }}_max,
            {%- else %}
            dateadd(
                'second', -1, dateadd('{{ time_grain }}', 1, {{ x_axis }}_min)
            ) as {{ x_axis }}_max,
            {%- endif %} {{ '\n        dimensions,' if group_by }}
            {%- for column, aggs in metrics.items() %}
            {%- set oloop = loop %}
...
with
    axis_range as (
        select min({{ x_axis }}) - 1 as min_val, max({{ x_axis }}) + 1 as max_val
        from {{ source_table }}
    ...
    buckets as (
        select
            ...
            {%- for column in metrics.keys() %} {{ column }}{{ ',' if not loop.last }}
            {%- endfor %}
        from {{ source_table }}
        cross join edges {{ filter_statement }}
    ),
    source_query as (
        select
            bucket,
    ...
    {%- if group_by %}
    bucket_spine as (
        select row_number() over (order by null) as bucket
        from table(generator(rowcount => {{ bucket_count }}))
    ),
    spine__values__dimensions as (select distinct dimensions from source_query),
    spine as (select * from bucket_spine cross join spine__values__dimensions),
    {%- else %}
    spine as (
        select row_number() over (order by null) as bucket
        from table(generator(rowcount => {{ bucket_count }}))
...
with
    ...
    tidy_data as (
        select
            {%- if not group_by or not flatten %} {{ x_axis }},
            {%- else %}
            {{ x_axis }} as {{ x_axis }}_min, {{ x_axis }} as {{ x_axis }}_max,
            {%- endif %} {{ '\n        dimensions,' if group_by }}
            {%- for column, aggs in metrics.items() %}
            {%- set oloop = loop -%}
            {%- for aggregation_type in aggs %}
            ...
            as {{ cleanse_name(aggregation_type + '_' + column) }}{{ '' if loop.last and oloop.last else ',' }}
            {%- endfor -%}
            {%- endfor %}
        from source_query
...
{%- else -%}
    ...
    pivoted__{{ metric_name }} as (
        ...
        from
            (
                select {{ x_axis }}_min, {{ x_axis }}_max, {{ metric_name }}, dimensions
                from tidy_data
            )
            pivot(
    ...
    pivoted as (
        ...
        left join
            pivoted__{{ metric_names[i] }}
            on x_min_{{ metric_names[0] }} = x_min_{{ metric_names[i] }}
            and x_max_{{ metric_names[0] }} = x_max_{{ metric_names[i] }}
        {%- endfor %}
    )
select
    {%- if axis_type == 'categorical' %} x_min_{{ metric_names[0] }} as {{ x_axis }},
    {%- else %}
    x_min_{{ metric_names[0] }} as {{ x_axis }}_min,
    x_max_{{ metric_names[0] }} as {{ x_axis }}_max,
    {%- endif %}
    {%- for column_name in column_names %} {{ column_name }}{{ ',' if not loop.last }}
    {%- endfor %}
from pivoted
order by 1 {{ x_axis_order if x_axis_order }}
{%- endif %}
