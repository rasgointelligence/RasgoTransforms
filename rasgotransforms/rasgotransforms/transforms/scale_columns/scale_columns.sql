...
with
    avg_stddev_vals as (
        select
            ...
            stddev(
                {{ column }}
            ) as stddev_{{ column }}{{ ", " if not loop.last else "" }}
            {%- endfor %}
        from {{ source_table }}
    )
select
    {{ source_table + ".*" if not overwrite_columns else untouched_cols }},
    {%- for column in columns_to_scale %}
    ({{ column }} - avg_{{ column }}) / (stddev_{{ column }})
    as {{ column if overwrite_columns else column + "_SCALED" }}{{ ", " if not loop.last else "" }}
    {%- endfor %}
...
select
    {{ untouched_cols }},
    {%- for column in columns_to_scale %}
    ({{ column }} - {{ averages[loop.index0] }}) / ({{ standarddevs[loop.index0] }})
    as {{ column if overwrite_columns else column + "_SCALED" }}{{ ", " if not loop.last else "" }}
    {%- endfor %}
from {{ source_table }}
{%- endif -%}
...
select
    {{ source_table + ".*" if not overwrite_columns else untouched_cols }},
    {%- for column in columns_to_scale %}
    ({{ column }} - min_{{ column }}) / (max_{{ column }} - min_{{ column }})
    as {{ column if overwrite_columns else column + "_SCALED" }}{{ ", " if not loop.last else "" }}
    {%- endfor %}
...
select
    {{ untouched_cols }},
    {%- for column in columns_to_scale %}
    ({{ column }} - {{ minimums[loop.index0] }}) / (
        {{ maximums[loop.index0] }} - {{ minimums[loop.index0] }}
    )
    as {{ column if overwrite_columns else column + "_SCALED" }}{{ ", " if not loop.last else "" }}
    {%- endfor %}
from {{ source_table }}
