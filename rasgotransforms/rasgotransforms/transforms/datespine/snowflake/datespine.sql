...
with
    calendar as (
        select
            ...
            cast(
                dateadd('day', interval_id -1, '{{ min_date }}'::timestamp_ntz) as date
            ) as date_day,
    ...
    spine as (select distinct date_{{ interval_type }} as period from calendar)
select
    cast(spine.period as timestamp) as {{ date_col }}_spine_start,
    {%- if interval_type|lower == 'quarter' %}
    dateadd(
        'second', -1, dateadd('month', 3, {{ date_col }}_spine_start)
    ) as {{ date_col }}_spine_end,
    {%- else %}
    dateadd(
        'second', -1, dateadd('{{ interval_type }}', 1, {{ date_col }}_spine_start)
    ) as {{ date_col }}_spine_end,
    {%- endif %}
    {{ source_table }}.*
from spine
left outer join
    ...
    on cast(
        date_trunc(
            '{{ interval_type }}', cast({{ source_table }}.{{ date_col }} as date)
        ) as date
    )
    = spine.period
