...
{%- if columns[column].drop -%} {{ drop_cols.append(column) or "" }} {%- endif -%}
{%- endfor -%}
{%- for col in drop_cols -%} {%- set _x = columns.pop(col) -%} {%- endfor -%}
...
{%- else -%} {%- set source_col = 'cast(' + name + ' as ' + col.type + ')' -%}
{%- endif -%}
{%- else -%} {%- set source_col = name -%}
{%- endif -%}
{% set output_col_name = name if col.name is not defined else cleanse_name(col.name) %}
{%- set impute_expression = 'NULL' -%}
{%- if col.impute is not defined -%} {%- set output = source_col -%}
{%- else -%}
