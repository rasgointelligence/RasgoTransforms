...
with
    ...
    joined as (
        select
            spine.period,
            {%- for dimension in dimensions %} spine.{{ dimension }}, {%- endfor %}
            {{ aggregation_type }} (
    ...
    tidy_data as (
        select
            ...
            {%- else %}
            dateadd(
                'second', -1, dateadd('{{ time_grain }}', 1, period_min)
            ) as period_max,
            {%- endif %}
...
{%- if not dimensions or not flatten %} select * from tidy_data order by period_min
{%- else -%}
    ,
    ...
    pivoted as (
        ...
        from
            (
                select period_min, period_max, {{ alias }}, dimensions
