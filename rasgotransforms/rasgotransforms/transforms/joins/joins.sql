{%- macro table_from_fqtn(fqtn) -%} {{ fqtn.split('.')[-1] }} {%- endmacro -%}
...
{%- if loop.index == 1 -%} {%- set base_table = source_table -%}
{%- else -%} {%- set base_table = join_dict['table_a'] -%}
{%- endif -%}
...
{%- else -%} {%- set ns.select_columns = ns.select_columns ~ ' ' ~ column -%}
{%- endif -%}
...
join
    ...
    {{ table_from_fqtn(source_table) if outer_loop.index == 1 else table_from_fqtn(join_dict["table_a"]) }}.{{ join_col1 }}
    = {{ table_from_fqtn(join_dict["table_b"]) }}.{{ join_col2 }}
    {% endfor %}
    {%- endif -%}
{%- endfor -%}
