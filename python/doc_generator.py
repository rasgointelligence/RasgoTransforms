...
def _get_transform_markdown(transform_yaml: Dict,
    ...
    markdown_elements = [
        ...
        md.h2('Example'),
        md.text(DATASET_PREVIEW_TEXT) if transform_yaml['preview-data'] else "",
        md.python_code(DATASET_PREVIEW_CODE) if transform_yaml['preview-data'] else "",
        md.text(transform_yaml['preview-data']) if transform_yaml['preview-data'] else "",
        md.text(TRANSFORMED_DATA_PREVIEW_TEXT) if transform_yaml['preview-data'] else "",
        md.python_code(transform_yaml['example_code']),
        md.text(transform_yaml['post-transform-data']) if transform_yaml['preview-data'] else "",
        md.h2('Source Code'),
